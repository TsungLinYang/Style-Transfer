...
from hand_tracker_3 import HandTracker

image_count = 0

class HiddenDisplayWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("隱藏視窗 B")
        self.resize(640, 480)
...
class StyleTransferApp(QWidget):
    def __init__(self):
        ...
        self.hidden_window = HiddenDisplayWindow()
        ...
        # B window
        self.toggle_b_btn = QPushButton("顯示/隱藏視窗 B")
        self.toggle_b_btn.clicked.connect(self.toggle_b_window)
        self.toggle_b_btn.setEnabled(True)
        ...
        # button_layout.addWidget(self.toggle_b_btn)
        button_layout.addStretch()
        ...
        # 移動視窗
        self.move(100, 100)  # A視窗移到螢幕左邊
        self.hidden_window.move(1200, 300)
        ...
        self.photo_time = 0
        self.sc_buffer = 0
        self.b_window_timer = 40
        self.is_clone = False
        
    def toggle_b_window(self):
        if self.hidden_window.isVisible():
    ...
    def update_frame(self):
        ...
        # 偵測手勢（加上冷卻）
        if self.hand_tracker.is_cross_finger_gesture(frame):
            self.hidden_window.show()
            self.is_clone = True
            
        if self.hidden_window.isVisible():
            self.b_window_timer -= 1
            if self.b_window_timer < 0:
                self.hidden_window.hide()
                self.b_window_timer = 40
        ...
        if self.is_clone == False:
            if time.time() - self.last_scisssort_time > self.scissort_cooldown_seconds and self.to_saved == 0:
                global image_count
        ...
        if time.time() >= self.photo_time and self.photo_time > 0:
            ...
            self.photo_time = -1
        if self.hidden_window.isVisible():
            self.hidden_window.update_image(stylized_frame)
